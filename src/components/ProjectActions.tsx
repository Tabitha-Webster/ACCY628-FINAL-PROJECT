...
export function ProjectActions({
  ...
  async function submitOutOfScope(e: React.FormEvent) {
    ...
    await run("change-request", async () => {
      ...
      const description = [
        ...
        crDescription.trim(),
        markRelatedTime
          ? "Related unbilled project time was also marked out of scope pending manager approval."
          : null,
      ]
        .filter(Boolean)
        .join("\n\n");
      ...
      setMessage("Out-of-scope work flagged with requested additional hours and price.");
      return null;
    });
  }
  ...
  return (
    <div className="space-y-4 rounded-box border border-base-300 bg-base-100 p-4">
      <p className="text-sm font-semibold">Project Actions</p>
...
export function ProjectChangeRequestPanel({
  ...
  const decided = requests.filter((r) => r.approval_status !== "pending");
    ...
    return (
      <div className="rounded-box border border-dashed border-base-300 bg-base-100 p-6 text-center">
        ...
        <p className="mt-1 text-sm opacity-70">
          When a technician flags work outside the approved project scope, it will appear here linked to the project and contract.
        </p>
      </div>
    );
  }
                ...
                >
                  ...
                  {canDecide(r) ? (
                    <div className="mt-3 space-y-2">
                      <p className="text-xs opacity-70">
                        {role === "customer"
                          ? "Approve or reject the requested additional hours and price for your organization."
                          : "Manager decision for this out-of-scope / change request."}
                      </p>
                      <textarea
                        className="textarea textarea-bordered textarea-sm w-full"
                        rows={1}
                    ...
                    <p className="mt-2">
                      <span className="badge badge-warning">
                        {role === "customer" ? "Pending internal review" : "pending manager approval"}
                      </span>
                    </p>
                  )}
                </div>
              ...
              return (
                <div key={r.id} className="rounded-box border border-base-300 bg-base-100 px-4 py-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className={`badge ${r.approval_status === "approved" ? "badge-success" : "badge-error"}`}>
                      {r.approval_status}
                    </span>
                  </div>
