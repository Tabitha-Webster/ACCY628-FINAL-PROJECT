...
export function ExportCustomersButton({ rows, role }: Props) {
  ...
  function onExport() {
    ...
    if (!canExportCustomers(role)) {
      setError(
        "Access denied. Only Manager and Billing & Accounting can export the customer list. HR, Technician, and Customer roles cannot export."
      );
      return;
    }
  ...
  if (!allowed) {
    return (
      <div className="alert alert-warning text-sm py-2 max-w-xl" role="alert">
        ...
        list. Only Manager and Billing & Accounting may export.
      </div>
    );
  }
