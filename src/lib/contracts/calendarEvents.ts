...
/** Pure builder used by Renewal & Expiration and the Contracts Dashboard. */
export function buildContractCalendarEvents(
  ...
  for (const reminder of reminders) {
    const contractJoin = reminder.contracts;
    ...
    events.push({
      ...
      label: reminder.message || reminderKindLabel(reminder.reminder_kind as never),
...
/** Build renewal / expiration / reminder events for the Contracts Dashboard calendar. */
export async function fetchContractCalendarEvents(supabase: SupabaseClient): Promise<{
  ...
  return {
    events: buildContractCalendarEvents(contracts, (remindersRes.data ?? []) as ReminderRow[]),
    error: null,
  };
}
