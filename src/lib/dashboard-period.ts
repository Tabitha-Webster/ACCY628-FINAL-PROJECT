...
export type DashboardView = "month" | "quarter" | "year";
...
export function parseDashboardView(value: string | string[] | undefined): DashboardView {
  ...
  if (view === "quarter" || view === "year" || view === "month") return view;
  return "month";
}
...
export function convertDashboardView(period: DashboardPeriod, nextView: DashboardView): DashboardPeriod {
  const current = parsePeriodDate(period.view, period.key);
  return resolveDashboardPeriod(nextView, periodKeyForDate(nextView, current), current);
}

export function dashboardPeriodOptions(view: DashboardView, selectedKey?: string, now = new Date()) {
  const count = view === "year" ? 6 : view === "quarter" ? 12 : 24;
  const options: { key: string; label: string }[] = [];
  for (let i = -2; i < count; i++) {
    const date =
      view === "quarter" ? addQuarters(startOfQuarter(now), -i) : view === "year" ? addYears(startOfYear(now), -i) : addMonths(startOfMonth(now), -i);
    options.push({ key: periodKeyForDate(view, date), label: compactPeriodLabelForDate(view, date) });
  }
...
export function periodViewControlProps(period: DashboardPeriod) {
  return {
    ...
    prevPeriodKey: shiftDashboardPeriod(period, -1).key,
    nextPeriodKey: shiftDashboardPeriod(period, 1).key,
    viewHrefs: {
      month: `?view=month&period=${convertDashboardView(period, "month").key}`,
      quarter: `?view=quarter&period=${convertDashboardView(period, "quarter").key}`,
      year: `?view=year&period=${convertDashboardView(period, "year").key}`,
    } as Record<DashboardView, string>,
    options: dashboardPeriodOptions(period.view, period.key),
  };
}
